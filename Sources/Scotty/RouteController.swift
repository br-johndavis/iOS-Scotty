...
open class RouteController<Navigatee>: NSObject {
    
    // MARK: Properties
	fileprivate let navigatee: Navigatee
    fileprivate(set) var isPreparedForRouting = false
	fileprivate(set) var storedRoute: (() -> Void)?
    
    // MARK: Initializers
    public init(navigatee: Navigatee, ready: Bool = true) {
        self.navigatee = navigatee
        super.init()
...
public extension RouteController {
    ...
    @discardableResult
    func open(_ route: Route<Navigatee>?, options: [AnyHashable: Any]? = nil) -> Bool {
        ...
        return route.route(fromNavigatee: navigatee, options: options)
    }
}
...
public extension RouteController {
	...
	enum StoredRoutePolicy {
		...
		fileprivate func executePolicy(with routeController: RouteController<Navigatee>) {
			switch self {
			case .execute:
...
fileprivate extension RouteController {
    ...
    func stored(route: Route<Navigatee>, options: [AnyHashable: Any]?) -> () -> Void {
        return { [weak self] in
